function  [delta_v, th_f, delta_t] = changeOrbitShapeALT(aI, eI, omI, aF, eF, omF, th0, option, rb)

% calcolo dv e dt non come vettori ma come scalari --> per non avere sbatti
% poi nei vari codici
...
if(nargin == 8)
    ...
    if(strcmp(option,'per'))
        delta_t(1) = timeOfFlight(aI,eI,rad2deg(th0), 0,mu); % Case pericenter
    else
        delta_t(1) = timeOfFlight(aI,eI,rad2deg(th0), 180,mu); % Case apocenter
    end
