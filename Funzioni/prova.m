    ...
    p1 = [10, 40] % downgrade of DIM time for improve convergency of the fimplicit method
    p2 = [-4, 35];
    ...
    [focalHyper, a, e, b, pos,th_rotation] = def_conic(p1,p2,a);
    
        % Extrapolate the points that belong to the hyperbola
    %fig = figure;
    tic
    fp = fimplicit(focalHyper, [-100 100 -100 100], 'MeshDensity',5000); % provare a risolvere definendo in un altro modo i punti dei rami di iperbole
    ...
    hold on
    %prova
           %Equation for Semi-Minor Axis, b
    a = abs(a)
    b = abs(b)
    
     x1 = linspace( abs(a), 10*abs(a), 5e3);         %From 0 to a, Upper
     ...
     x = [x1'; x1'; -x1'; -x1'];
     point = [ytop'; ybot'; ytop'; ybot'];
     
     hyper = [x, point]*R(deg2rad(90 + th_rotation));
     hyper(:,1) = hyper(:,1) + pos(1);
     hyper(:,2) = hyper(:,2) + pos(2);
     toc
     plot(hyper(:,1), hyper(:,2), 'g')
     grid on

     %%
    x1 = linspace(   abs(a), 10*abs(a), 1E3);         %From 0 to a, Upper
     ...
     hyper =R(deg2rad(th_rotation))*[x; point];
     hyper(1,:) = hyper(1,:) + pos(1);
     hyper(2,:) = hyper(2,:) + pos(2);

     scatter(hyper(1,:), hyper(2,:))
     grid on
...
% - th2     is the true anomlay of r2 in kep2
    ...
    x = [x1'; x1'; -x1'; -x1'];
    point = [ytop'; ybot'; ytop'; ybot'];
    %if(th_rotation > 0) % CAPIRE BENE QUESTA CONDIZIONE POI DOVREBBE ESSERE FATTA!!!!!
    %    focal = [x, point]*R(deg2rad(th_rotation)); %se sfaso di 90 viene forse perchè la rotazione del sistema cambia il riferimento degli angoli dalla verticale all'orizzontale
    %else
    focal = R(deg2rad(th_rotation + correction))*[x'; point']; % matrice di rotazione oraria
    %end
    %focal(:,1) = focal(:,1) + pos(1);
    %focal(:,2) = focal(:,2) + pos(2);

    focal(1,:) = focal(1,:) + pos(1);
    focal(2,:) = focal(2,:) + pos(2);
    
    %hold on
    %plot(focal(1,:), focal(2,:))
